...
{	key_t		semkey, shmkey;
	...
	{	retry=1;
		...
		{	snprintf(buffer, sizeof(buffer)-1,
					"Process: %s, PID: %d, Loop: %d\n",
					argv[0], getpid(), j);
			LOCK_SEMAPHORE(semid);
			if ( (MAXBUFFER-strlen(shmem->buf)) >= strlen(buffer) )
			{	printf("Transmitting string: %s", buffer);
				strncpy(shmem->buf+strlen(shmem->buf),
